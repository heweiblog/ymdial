...
int g_betbuf_size = sizeof(netbuf);
...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl)
{
		...
		tm.tv_sec  = 0; 
		tm.tv_usec = 1000*100;
		setsockopt(sControl, SOL_SOCKET, SO_SNDTIMEO, &tm, sizeof(struct timeval));

		tm.tv_sec  = 0; 
		tm.tv_usec = 1000*500;
		setsockopt(sControl, SOL_SOCKET, SO_RCVTIMEO, &tm, sizeof(struct timeval));
		...
		ctrl = (netbuf*)calloc(1,sizeof(netbuf)); /* 分配内存并初始化为0，*/
		if (ctrl == NULL)
		{
				if (ftplib_debug)
						perror("calloc");
...
int login_ftp(const char*host,const char* user,const char* pass)
{
		netbuf *nControl = NULL;
		int rtn = FtpConnect(host,&nControl);
		if(!rtn)
		{
				return rtn;
		}

		rtn = FtpLogin(user,pass,nControl);
